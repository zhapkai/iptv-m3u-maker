...
class Tools (object) :
    ...
    def getRealUrl(self, url):
        if url.startswith("="):
            self.logger(f"开始解码 (V5模式), 原始URL: {url[:70]}...", level="INFO")
            try:
                ...
                # ================== 【V5 最终解密核心】 ==================
                # 步骤 2: 使用“初始密钥材料”的MD5哈希值作为最终的RC4密钥
                final_rc4_key = hashlib.md5(initial_key_material).digest()
                self.logger(f"派生出最终RC4密钥 (初始密钥的MD5): {final_rc4_key.hex()}", level="DEBUG")
                ...
                if md5_of_result == md5_from_payload:
                    ...
                    self.logger("MD5验证成功！V5最终方案破解成功！", level="SUCCESS")
