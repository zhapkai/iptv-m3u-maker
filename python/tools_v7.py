...
class Tools (object) :
    ...
    def getRealUrl(self, url):
        if url.startswith("="):
            ...
            try:
                # 步骤 1: Base64解码 + 第一次异或(XOR)解密，得到“加密包裹”
                encoded_str = url[1:]
                ...
                # ==================== 【核心逻辑变更】 ====================
                # 步骤 3: 使用“初始密钥”(Key_A)，而不是MD5，进行第二次异或(XOR)解密
                
                initial_key_len = len(initial_key_bytes)
                decrypted_url_bytes = bytearray()
                for i in range(len(encrypted_part)):
                    decrypted_byte = encrypted_part[i] ^ initial_key_bytes[i % initial_key_len]
                    decrypted_url_bytes.append(decrypted_byte)
                
                # =========================================================

                # 步骤 4: 验证！计算解密出的URL的MD5，与载荷中的MD5进行比对
                ...
                import traceback
                self.logger(traceback.format_exc(), level="DEBUG") # 打印更详细的错误堆栈
                return ""
        else:
            return url
