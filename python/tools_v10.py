...
class Tools (object) :
    ...
    def getRealUrl(self, url):
        if url.startswith("="):
            self.logger(f"开始解码 (RC4模式), 原始URL: {url[:70]}...", level="INFO")
            try:
                # 步骤 1: 分离出“密钥”和“加密数据”
                encoded_str = url[1:]
                ...
                # 这是RC4算法的密钥
                rc4_key_bytes = base64.b64decode(pad_base64(pattern_key_encoded))
                # 这是需要解密的完整载荷
                encrypted_payload = base64.b64decode(pad_base64(url_fragment_encoded))
                
                # ==================== 【全新RC4解密核心】 ====================
                # 步骤 2: 使用密钥，通过RC4算法一次性解密整个载荷
                decrypted_payload = self._rc4_decrypt(rc4_key_bytes, encrypted_payload)
                self.logger("RC4解密完成，开始验证MD5...", level="DEBUG")
                # ==========================================================

                # 步骤 3: 检查解密后长度，并分离“MD5指纹”和“URL”
                ...
                if md5_of_result == md5_from_payload:
                    ...
                    self.logger("MD5验证成功！RC4算法破解成功！", level="SUCCESS")
            ...
            # 对于非 "=" 开头的URL，直接返回，或者根据需要处理
            # 在主程序中已经有跳过逻辑，这里作为双重保险
            return url
